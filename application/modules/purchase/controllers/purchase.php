...
class Purchase extends CI_Controller {
	...
	function po( $action=NULL ) {
		...
		if ( $action == 'c' ) {
			...
			try {
				...
				$doc_code = get_doc_code($company_id, $branch_id, $department_id, 'PO');
				if (!$doc_code)
				...
				$this->db->insert('po', $data1);
				
				$result = set_doc_last_number($company_id, $branch_id, $department_id, 'PO', $doc_code);
				if (!$result)
		...
		if ( $action == 'u' ) {
			...
			try {
				...
				$this->db->update( 'po', $data1, array('id'=>$data['id']) ); 
			...
			$this->db->trans_commit();
			$this->shared_model->set_comet('po');
			echo json_encode(array("success"=>1));
			return;
		}
		...
		if ( $action == 'd' ) {
			...
			try {
				...
				// $this->db->delete( 'po_dt', array('po_id'=>$data['id']));
				// $this->db->delete( 'po', array('id'=>$data['id']));
		...
		if ( $action == 'recalc' ) {
			...
			try {
				...
				$result = $this->shared_model->recalc_transaction_per_id( 'po', $data['id'] );
	...
	function po_dt( $action=NULL, $po_id=0 ) {
		if (!$this->ion_auth->logged_in())
			redirect('main', 'refresh');
		...
		if ( $action == 'u' ) {
			...
			// $row = $this->db->get_where( 'po_dt', array('id'=>$data['id']) )->row();
			...
			$rate = $this->shared_model->get_currency_rate($data['currency_id']);
			if ( $rate==FALSE )
				crud_error("error: currency not defined !");
			...
			try {
				...
				$data1['measure_id'] 	= $data['measure_id'];
				$data1['currency_id'] 	= $data['currency_id'];
				$data1['currency_rate'] = $rate;
				$data1['disc_percent'] 	= $data['disc_percent'];
				$data1['disc_amount'] 	= $data['disc_amount'];
				$data1['sub_total'] 	= $data['sub_total'];
				$this->db->update( 'po_dt', $data1, array('id'=>$data['id']) );
		...
		if ( $action == 'd' ) {
			...
			try {
				...
				$this->db->delete( 'po_dt', array('id'=>$data['id']) );
	...
	function form_phd_pur_princ( $id=NULL ) {
