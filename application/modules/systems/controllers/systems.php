...
class Systems extends CI_Controller {
	...
	function get_themes() {
		...
		// $row = $this->db->get_where('users_themes', array('user_id'=>$this->session->userdata('user_id')), 1)->row();
		// if ( !isset($theme->theme_id) ) 
		// {
			// $this->db->insert('users_themes', array('user_id'=>$this->session->userdata('user_id'), 'themes_code'=>'black'));
		// }
		// $users_themes = (isset($row->themes_code)) ? $row->themes_code : 'black';
		
		// $rows = $this->db->query("select * from themes")->result();
		// $i=0;
		// foreach ($themes as $key => $row) {
			// $options[$key] = $row;
			// if ($row->code == $users_themes) 
		...
		$theme = $this->systems_model->getTheme_ByUserId($user_id);
	...
	function set_themes() {
		...
		$this->systems_model->updateTheme_ByUser($data, $user_id);
	}
	...
	function reset_pwd( $action=NULL )
	{
		...
		$change = $this->ion_auth->reset_password( $identity, $data['pwd'] );
	...
	function users( $action=NULL ) {
		...
		if ( $action == 'c' ) {
			...
			$data2['user_id'] = $id;
			$data2['theme_id'] = 1;
			$this->systems_model->addUsers_Settings($data2);
		...
		if ( $action == 'd' ) {
			...
			// if ( $this->shared_model->is_data_exists_on( 'phd_routes', array('checkby', 'signby', 'createby', 'updateby'), $id) ) {
				// echo json_encode( array("errorMsg"=>l('error_data_transaction')) );
				// return;
			// } 
			// if ( $this->shared_model->is_data_exists_on( 'phd', array('sellerby', 'checkby', 'signby', 'createby', 'updateby'), $id) ) {
				// echo json_encode( array("errorMsg"=>l('error_data_transaction')) );
				// return;
			// } // {end}
	...
	function groups( $action=NULL )	{
		...
		if ( $action == 'd' ) {
			...
			try {
				...
				$this->db->delete('groups_auth', array('group_id'=>$id));
	...
	function groups_auth( $action=NULL ) {
		...
		if ( $action == 'u' ) {
			...
			try {
				...
				$qry = $this->db->get_where( 'groups_auth', $data1, 1 );
				...
				if ( $qry->num_rows > 0 ) 
				{
					$this->db->update('groups_auth', $data1, array('id'=>$qry->row()->id));
				}
				else 
				{
					$this->db->insert('groups_auth', $data1);
				} 
	...
	function company( $action=NULL ) {
		...
		if ( $action == 'd' ) {
			...
			// {begin} check if this data has already have transaction
			// if ( $this->shared_model->is_data_exists_on( 'users_company', array('company_id'), $id) ) {
				// echo json_encode( array("errorMsg"=>l('error_data_transaction')) );
				// return;
			// } 
			// if ( $this->shared_model->is_data_exists_on( 'phd', array('company_id'), $id) ) {
				// echo json_encode( array("errorMsg"=>l('error_data_transaction')) );
				// return;
			// } 
			// {end}
	...
	function branch( $action=NULL ) {
		...
		if ( $action == 'd' ) {
			...
			// {begin} check if this data has already have transaction
			// if ( $this->shared_model->is_data_exists_on( 'users_branch', array('branch_id'), $id) ) {
				// echo json_encode( array("errorMsg"=>l('error_data_transaction')) );
				// return;
			// } 
			// {end}
	...
	function modules_groups( $action=NULL )	{
		...
		if ( $action == 'c' ) {
			...
			try {
				...
				$data1['show'] 	  = array_key_exists('show', $data) ? 1 : 0;
				$this->db->insert($mdl, $data1);
	...
	function modules( $action=NULL ) {
		...
		if ( $action == 'd' ) {
			...
			$module = $this->db->get_where('modules', array('id'=>$data['id']))->row();
			
			// ============= CRUD SECTION
			$this->db->trans_begin();
			try {
			
				$this->db->delete( 'groups_auth', array('module_id'=>$data['id']) );
				$this->db->delete( 'modules', array('id'=>$data['id']) );
				
				// RE-ORDER
				$qry = $this->db->order_by('sort_no')->get_where('modules', array('module_group_id'=>$module->module_group_id));
				$sort_no = 1;
				foreach ($qry->result() as $row)
				{
	...
	function setup_documents( $action=NULL ) {
		...
		if ( $action == 'd' ) {
			...
			// if ( $this->shared_model->is_data_exists_on( 'users_branch', array('branch_id'), $id) ) {
				...
				// return;
			// } 
			// if ( $this->shared_model->is_data_exists_on( 'phd', array('branch_id'), $id) ) {
				// echo json_encode( array("errorMsg"=>l('error_data_transaction')) );
				// return;
			// } 
			// {end}
			...
			try {
				...
				$this->db->delete( 'setup_documents', array('id'=>$data['id']) );
